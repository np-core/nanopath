...
# Main method
def merge_reports(
    r_files: list, out: Path = None, headers: bool = True,
    s_names: list = tuple(), c_only: bool = False
):

    if out is None:
        # Use temp file and read with pandas
    ...
    map_lvls = {'kingdom': 'K', 'superkingdom': 'D', 'phylum': 'P', 'class': 'C', 'order': 'O', 'family': 'F',
                'genus': 'G', 'species': 'S'}
    count_samples = 0
    num_samples = len(r_files)
    ...
    o_file.write(f"{(float(u_reads[0]) / float(total_reads[0]) * 100)}\t")
    for i in u_reads:
        if i == 0 or (i > 0 and not c_only):
    ...
    while len(all_nodes) > 0:
        ...
        o_file.write(f"{(float(curr_node.tot_all) / float(total_reads[0]) * 100)}\t")
    ...
    if out is None:
        if c_only:
            report = pandas.read_csv(
                output, sep='\t', comment='#', header=None,
