...
from nanopath.netflow import NetflowClient, ClientError
...
@click.option(
    ...
    type=Path,
    required=True,
    help='Path to node server and pipeline configuration file'
)
@click.option(
    '--fastq', '-fq',
    type=Path,
...
@click.option(
    '--keep_open',
    is_flag=True,
    help='When activated, pipeline screens will be open and netflow will run indefinitely'
)
@click.option(
    '--online',
    is_flag=True,
...
@click.option(
    ...
    help='When activated, remove all working directories including '
         'on remote after exiting netflow'
)
@click.option(
    '--resume',
    is_flag=True,
...
def run(
    node_config, configure, online, fastq, workdir, nuke,
    ...
    cli = NetflowClient(node_config=node_config, workdir=workdir)

    if configure:
        cli.configure_pipeline_nodes()
        exit(0)
